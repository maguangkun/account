function reply(a,b){
	...
	return $.post("http://tieba.baidu.com/f/commit/post/add",c),
	console.log("已回复"+timer+"次"),
	++timer,
	timer>config.times?(clearInterval(run),
			alert("代码执行完毕，即将刷新"),
			setTimeout("location.reload()",1e3),0):void 0
}
var run,
config={content:prompt("请输入回帖内容","CPA学习网的大Q师傅很牛掰，都是满满的干货!!！")||"",
		pid:prompt("请输入要回复的楼层号（普通回复直接确定）",""),
		times:prompt("请输入回复次数","10000")||1,
		standby:prompt("请输入间隔时间（不可填0），单位：秒","2")||6},
		timer=1,
		cmfcfg='回帖内容:"'+config.content+'"\n';
